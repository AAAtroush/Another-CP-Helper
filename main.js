...
function importCFInput() {
  ...
  const matrix = lines.map(l => l.split(/\s+/).map(v => (v === '' ? '' : Number(v))));
...
function generateRandomGrid() {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const cell = document.querySelector(`[data-row="${r}"][data-col="${c}"]`);
      if (cell) cell.querySelector('input').value = Math.floor(Math.random() * 100);
    }
  }
  saveHistory();
}
...
function loadTemplate() {
  ...
  if (!obj || !obj.rows || !obj.cols || !obj.data) return alert('Template corrupted.');
  isInitializing = true;
...
function toggleInputs() {
  ...
  document.querySelectorAll('.cell input').forEach(inp => inp.disabled = inputsDisabled);
  toggleInputsBtn.textContent = inputsDisabled ? 'Enable Inputs' : 'Disable Inputs';
}
...
function rotateGrid(clockwise = true) {
  ...
  if (clockwise) {
    ...
    newMatrix = matrix[0].map((_, i) => matrix.map(row => row[row.length - 1 - i]));
  }
...
document.querySelectorAll(".formulas-tab-btn").forEach(tab => {
  tab.addEventListener("click", () => {
    document.querySelectorAll(".formulas-tab-btn").forEach(t => t.classList.remove("active"));
    document.querySelectorAll(".formula-section").forEach(p => p.classList.remove("active"));
